      ...
      <div className="space-y-6">
        ...
        {modalOpen && (
          <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-emerald-900/60 rounded-2xl w-full max-w-2xl p-6 shadow-2xl relative space-y-4 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center border-b border-emerald-900/40 pb-3">
                <h3 className="text-lg font-bold text-emerald-400 flex items-center gap-2">
                  <Truck className="w-5 h-5" /> Dispatch New Sprouts Delivery
                </h3>
              ...
              <form onSubmit={handleCreateDelivery} className="space-y-4">
                ...
                {/* Items Builder */}
                <div className="space-y-2 border-t border-b border-emerald-900/40 py-3">
                  ...
                  {items.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      ...
                      <div className="w-24 text-right text-xs font-bold text-emerald-300">
                ...
                {/* Calculation Summary */}
                <div className="bg-slate-800/60 p-4 rounded-xl space-y-2 text-xs text-slate-300">
        ...
        {invoiceModalDelivery && (
          <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-emerald-900/80 rounded-2xl w-full max-w-xl p-6 shadow-2xl space-y-4">
              <div className="flex justify-between items-center border-b border-emerald-900/40 pb-4">
                <div>
                  <h3 className="font-bold text-lg text-emerald-400">MamaFarm Organic Sprouts</h3>
                  <p className="text-xs text-slate-400">Tax Invoice & Delivery Receipt</p>
                </div>
                <button onClick={() => setInvoiceModalDelivery(null)} className="text-slate-400 hover:text-white">
                  <X className="w-5 h-5" />
                </button>
              </div>

              <div className="grid grid-cols-2 text-xs gap-4 text-slate-300 bg-slate-800/40 p-4 rounded-xl border border-emerald-900/30">
                <div>
              ...
              <div className="border border-emerald-900/40 rounded-xl overflow-hidden">
                <table className="w-full text-left text-xs text-slate-300">
                  <thead className="bg-slate-800 text-emerald-400">
                    <tr>
                      <th className="p-3">Sprout Item</th>
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {invoiceModalDelivery.items?.map((it, idx) => (
                      <tr key={idx}>
                        <td className="p-3 text-white">{it.sproutType}</td>
                        <td className="p-3">{it.quantity} {it.unit}</td>
                        <td className="p-3">₹{it.rate}</td>
                        <td className="p-3 text-right font-semibold text-white">₹{it.amount}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex justify-between items-center text-sm font-bold bg-emerald-950/60 p-4 rounded-xl border border-emerald-800/50 text-white">
                <span>Total Amount Payable:</span>
                <span className="text-emerald-400 text-lg">₹{invoiceModalDelivery.netAmount}</span>
              </div>

              <div className="flex justify-between items-center pt-3 border-t border-emerald-900/40">
                <button
                  onClick={() => window.print()}
                  className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-emerald-300 rounded-xl text-xs font-semibold flex items-center gap-2"
                >
